...
def get_tests(public_samples):
    """
    Returns a list of test files. 
    
    If `public_samples` is a directory, this function returns all GLSL files 
    that are under that folder. 

    Otherwise, `public_samples` is treated as a glob pattern, and all 
    matching files are returned. 
    """

    if path.isdir(public_samples):
        return glob.glob(path.join(public_samples, GLSL_PATTERN))
    else:
        tests = filter(path.isfile, glob.glob(public_samples))
        if not tests:
            raise argparse.ArgumentTypeError(
                "Could not find test(s): {0}".format(public_samples))
        return tests
